...
def testdiversify_results_limits_repeated_source_page():
    results = [
...
def testdiversify_results_fills_when_constraints_tight():
    results = [
...
def testdiversify_results_can_be_disabled():
    results = [
        {"id": "a", "source": "s1.pdf", "page": 1, "content": "x1"},
        {"id": "b", "source": "s1.pdf", "page": 1, "content": "x2"},
        {"id": "c", "source": "s1.pdf", "page": 1, "content": "x3"},
    ]
    diversified = diversify_results(results, top_k=2, enable_diversification=False)
...
def test_rank_documents_bm25_only_does_not_drop_docs_when_embeddings_missing():
    ranked = rank_documents(
        documents={
            "ids": ["a", "b"],
            "contents": ["LDL target guidance", "diabetes meal planning"],
            "embeddings": [],
            "metadatas": [{"source_class": "guideline_pdf"}],
        },
        keyword_scores={0: 2.0, 1: 1.0},
    ...
    assert [row["idx"] for row in ranked] == [0, 1]
    assert ranked[0]["source_prior"] > ranked[1]["source_prior"]
    assert ranked[1]["source_prior"] == pytest.approx(0.05)


def test_rank_documents_semantic_requires_embedding_count_match():
    with pytest.raises(ValueError, match="Embedding count does not match"):
        rank_documents(
            documents={
                "ids": ["a", "b"],
