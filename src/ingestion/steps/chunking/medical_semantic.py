...
Layers medical-aware section pre-splitting and merge rules on top of
...
class MedicalSemanticChunkerAdapter(ChonkieChunkerAdapter):
    ...
    - Pre-splitting at medical section boundaries (clinical/guideline headers)
    ...
    def _should_merge_chunks(self, chunk1: dict[str, Any], chunk2: dict[str, Any]) -> bool:
    ...
    def _post_process_chunks(self, chunks: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        while i < len(chunks):
            ...
            if i + 1 < len(chunks) and self._should_merge_chunks(current, chunks[i + 1]):
                # Merge current and next
                merged = self._merge_two_chunks(current, chunks[i + 1])
    ...
    def chunk_text(
        ...
        """Chunk text with medical-aware boundary handling.
        ...
        split_positions = self.structure_rules.get_split_positions(text)
        if split_positions:
            all_chunks = []
            prev_pos = 0
            chunk_offset = 0
            ...
            base_chunks = super().chunk_text(text, source, doc_id, page)

        # Post-process to preserve medical structure
        chunks = self._post_process_chunks(base_chunks)
